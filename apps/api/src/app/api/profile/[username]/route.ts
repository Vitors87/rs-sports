...
function computeAchievements(
  ...
  if (activities.some((a) => a.sport.type === 'RUNNING' && (a.distance ?? 0) >= 10)) {
    result.push({
      icon: '🏅',
      title: 'Primer 10K',
      description: 'Completó una actividad de running de al menos 10 km',
    });
  }
  ...
  if (activities.filter((a) => a.sport.type === 'TREKKING').length >= 3) {
    result.push({
      icon: '⛰️',
      title: 'Trekker activo',
      description: 'Tiene 3 o más actividades de trekking registradas',
    });
  }
  ...
  if (cyclingKm >= 100) {
    result.push({
      icon: '🚴',
      title: 'Ciclista frecuente',
      description: 'Acumuló 100 km o más en ciclismo',
    });
  }
  ...
  if (activities.filter((a) => a.sport.type === 'RUNNING').length >= 5) {
    result.push({
      icon: '🏃',
      title: 'Runner constante',
      description: 'Tiene 5 o más actividades de running registradas',
    });
  }
  ...
  if (['RUNNING', 'CYCLING', 'TREKKING'].every((t) => sportTypes.has(t))) {
    result.push({
      icon: '🌿',
      title: 'Explorador outdoor',
      description: 'Tiene actividades en las 3 disciplinas',
    });
  }
